...
Installs dependencies and starts the Bun server.
"""
...
def main():
    ...
    if not os.path.exists(os.path.join(server_dir, "node_modules")):
        ...
        subprocess.run(["bun", "install"], cwd=server_dir, check=True)
    ...
    if not os.path.exists(os.path.join(client_dir, "dist")):
        if not os.path.exists(os.path.join(client_dir, "node_modules")):
            ...
            subprocess.run(["bun", "install"], cwd=client_dir, check=True)
        print("Building client...")
        subprocess.run(["bun", "run", "build"], cwd=client_dir, check=True)
    
    # Start the server
    print("Starting server on http://localhost:3000")
    subprocess.run(["bun", "run", "start"], cwd=server_dir)

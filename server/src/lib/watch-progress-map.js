...
export function createWatchProgressStore(dataDir) {
  const mapPath = path.join(dataDir, "watch-progress-map.json");
  ...
  async function writeMapAtomic(map) {
    ...
    const tempPath = path.join(
      dir,
      `.watch-progress-map.${process.pid}.${Date.now()}.${Math.random().toString(16).slice(2)}.tmp`
    );
  ...
  async function updateWatchProgress(mutator) {
  ...
  return { mapPath, readWatchProgress, updateWatchProgress };
}
